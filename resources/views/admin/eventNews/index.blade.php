        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-12 col-md-12 col-lg-12">
                    <a style="float: right;" href="{{URL::to('admin/event-news/create')}}" class="btn btn-primary pull-right"> Add Event News</a>
                </div>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if(!empty($row->txtuplode))
                                                        <img style="margin: 5%;" class="w-25 img-responsive" alt="image" id="logoimg" src="{{ URL::asset('public/upload/admin/cmsfiles/events/thumbnail/')}}/{{$row->txtuplode}}" class="rounded-circle mr-1" />
                                                    @endif
                                                    <input type="hidden" name="oldimg" value="{{ !empty($row->txtuplode)?$row->txtuplode:''}}" >
                                                </td>
                                                <td class="d-flex" style="padding: 7% 14%;">
                                                    <form action="{{ route('event-news.destroy',$row->id) }}"  method="POST"> 
                                                        <a class="btn btn-primary" href="{{ route('event-news.edit',$row->id) }}">Edit</a>
                                                        @csrf
                                                        @method('DELETE')
